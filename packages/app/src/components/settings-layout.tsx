          ...
          <SettingsList>
            <div class="flex flex-wrap items-center gap-4 py-3.5 border-b border-border-weaker-base last:border-none sm:flex-nowrap">
              <div class="flex min-w-0 flex-1 flex-col gap-1">
                <span class="text-14-medium text-text-strong">{language.t("settings.layout.showAvatar.title")}</span>
                <span class="text-12-regular text-text-weak leading-5">{language.t("settings.layout.showAvatar.description")}</span>
              </div>
              <div class="flex w-full justify-end sm:w-auto sm:shrink-0">
                <Switch
                  checked={settings.layout.showAvatar()}
                  onChange={(checked) => settings.layout.setShowAvatar(checked)}
                />
              </div>
            </div>
          </SettingsList>
